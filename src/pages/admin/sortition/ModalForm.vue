      ...
      <template #header>
        <div class="tw-flex tw-justify-between">
          ...
          <div class="" v-if="Type.EDIT === actionType">
            <va-button color="danger" @click="deleteSortition()"> Excluir </va-button>
          </div>
        </div>
      </template>
      <slot>
        <div class="tw-w-full tw-max-w-2xl tw-mt-4">
          <va-form ref="form" @validation="validation = $event">
            ...
            <div class="line tw-flex tw-w-full tw-mb-2">
              <div class="tw-w-1/2 tw-mx-1">
                <va-date-input v-model="formData.date" label="Data do sorteio" />
              </div>
              <div class="tw-w-1/2 tw-mx-1">
                <va-input v-model="formData.reward" :rules="fieldsValidations.required" type="text" label="Premiação">
                </va-input>
              </div>
            </div>
            ...
            <div class="line">
              <div class="tw-w-1/2 tw-mx-1">
                <va-switch
                  ...
                  true-inner-label="Aberto"
                  false-inner-label="Fechado"
                  class="mr-4"
                />
              </div>
            </div>
    ...
    computed: {
      options() {
        if (this.actionType === this.Type.EDIT) {
          return this.$store.state.NFTModule.nftSummaryIdList.map(
            (nftItem) =>
              ({
                text: `${nftItem.name} - ${nftItem.id.substring(0, 3)}...`,
                value: nftItem.id,
                id: nftItem.id,
                name: nftItem.name,
              } as any),
          ...
          return this.$store.state.NFTModule.nftSummaryIdList
            .map(
              (nftItem) =>
                ({
                  text: `${nftItem.name} - ${nftItem.id.substring(0, 3)}...`,
                  vinculated: nftItem.vinculated,
                  value: nftItem.id,
                  id: nftItem.id,
                  name: nftItem.name,
                } as any),
            )
            .filter((nft) => !nft.vinculated)
        }
      },
    },
  })
