<template>
  <div :style="{ maxWidth: nftMeasurement.NFTWidth + 'px' }" class="tw-overflow-x-auto">
    <div
      :style="{ minWidth: nftMeasurement.NFTWidth + 'px' }"
      class="tw-bg-gray-200 tw-m-auto tw-flex tw-flex-wrap tw-shadow-2xl"
    >
      <div v-for="(chunk, i) in nftMeasurement.nft.chunks" :key="i" class="tw-flex tw-flex-wrap">
        <div
    ...
    <va-modal v-model="showModal" hide-default-actions overlay-opacity="0.2">
      ...
      <slot>
        <div v-if="themes().length > 0" class="tw-flex tw-justify-between tw-mt-3">
          <div
      ...
      <template #footer>
        <div class="tw-flex tw-flex-col">
          ...
          <va-button class="tw-mt-2" flat size="small" @click="clearAll()"> Limpar todos </va-button>
        </div>
      </template>
    </va-modal>
  </div>
  ...
  export default defineComponent({
    ...
    methods: {
      ...
      clearPixelSelection() {
        ...
        this.pixelsSelectedForBuy = this.pixelsSelectedForBuy.filter((pixel) => pixel.uuid !== this.pixelSelected.uuid)
      ...
      updatePixelsFromCartToNFT(cart: Cart) {
        ...
        if (cart?.pixels?.length > 0) {
          cart.pixels.forEach((cartPixel) => {
            const pixelRef = this.nftMeasurement?.nft.chunks[cartPixel.chunkPosition].pixels[cartPixel.position]
            if (pixelRef) {
