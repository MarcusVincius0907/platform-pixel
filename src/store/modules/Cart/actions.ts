...
export const CartAction = {
  async [ActionTypes.GET_CART](context: ActionContext<CartModuleState, State>, sortitionId: string) {
  ...
  async [ActionTypes.CREATE_CART](context: ActionContext<CartModuleState, State>, sortitionId: string) {
    const CartService = await import('@/services/cartService')
  ...
  async [ActionTypes.UPDATE_CART](context: ActionContext<CartModuleState, State>, pixels: Array<Pixel>) {
    const CartService = await import('@/services/cartService')
    ...
    if (resp.status === ResponseStatus.OK) {
      context.dispatch(ActionTypes.GET_CART, context.state.cart?.sortitionId ?? '')
    } else {
      console.error('Cart error', resp)
    }
  },

  async [ActionTypes.DELET_CART](context: ActionContext<CartModuleState, State>) {
    const CartService = await import('@/services/cartService')
