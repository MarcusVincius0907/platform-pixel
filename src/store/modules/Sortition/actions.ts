...
export const SortitionAction = {
  async [ActionTypes.GET_SORTITION_LIST](context: ActionContext<SortitionModuleState, State>) {
  ...
  async [ActionTypes.GET_SORTITION_BY_ID](context: ActionContext<SortitionModuleState, State>, sortitionId: string) {
    context.commit(MainMutationsType.SET_CUSTOM_LOADER, true)
  ...
  async [ActionTypes.CREATE_SORTITION](context: ActionContext<SortitionModuleState, State>) {
  ...
  async [ActionTypes.UPDATE_SORTITION](context: ActionContext<SortitionModuleState, State>) {
    if (context.state.formDataSortition) {
      context.commit(MainMutationsType.SET_CUSTOM_LOADER, true)
  ...
  async [ActionTypes.DELETE_SORTITION](context: ActionContext<SortitionModuleState, State>) {
    if (context.state.formDataSortition) {
      context.commit(MainMutationsType.SET_CUSTOM_LOADER, true)
  ...
  async [ActionTypes.DEFINE_SELECTED_SORTITION_RANDOMLY](context: ActionContext<SortitionModuleState, State>) {
    await context.dispatch(ActionTypes.GET_SORTITION_LIST)

    if (context.state.sortitionList.length > 0) {
      context.commit(MutationsType.SET_SELECTED_SORTITION, context.state.sortitionList[0])
    }
  },
}
